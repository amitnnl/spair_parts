                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                    ${[
                        ...
                        <div class="bg-white rounded-3xl p-8 ring-1 ring-slate-900/5 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_40px_rgb(0,0,0,0.08)] group transition-all duration-500 hover:-translate-y-2 relative overflow-hidden">
                            ...
                            <p class="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-6 relative z-10">${escapeHTML(s.l)}</p>
                            <div class="flex items-end justify-between relative z-10">
                                <h3 class="text-4xl font-black text-slate-900 tracking-tight">${escapeHTML(s.v)}</h3>
                                <span class="text-[11px] font-black text-${escapeHTML(s.c)}-700 bg-${escapeHTML(s.c)}-50/50 backdrop-blur-sm px-3 py-1.5 rounded-xl border border-${escapeHTML(s.c)}-100/50 shadow-sm">${escapeHTML(s.s)}</span>
                            </div>
                        </div>
                    `).join('')}
                </div>
                ...
                <div class="bg-slate-900 rounded-[2.5rem] p-12 text-white relative overflow-hidden shadow-[0_20px_50px_rgb(0,0,0,0.15)] ring-1 ring-white/10">
                    ...
                    <div class="relative z-10 max-w-2xl">
                        ...
                        <div class="flex flex-wrap gap-4">
                            <button onclick="app.renderBulkOrderModal()" class="px-8 py-4 bg-white text-bosch-blue rounded-3xl font-black text-[11px] uppercase tracking-widest shadow-xl hover:scale-105 transition-transform flex items-center gap-2">
                            ...
                            <button onclick="app.renderCatalog(document.getElementById('view-container'))" class="px-8 py-4 bg-bosch-blue text-white rounded-full font-black text-[11px] uppercase tracking-widest shadow-xl shadow-slate-900/30 hover:bg-slate-700 transition-colors">Browse Catalog Manually</button>
                        </div>
                    </div>
                </div>
                </div>
...
export function renderBulkOrderModal(app) {
    ...
    modal.innerHTML = `
        <div class="bg-white rounded-3xl w-full max-w-xl p-8 space-y-6 shadow-2xl border border-slate-200 relative animate-in zoom-in-95 duration-300">
            <button onclick="document.getElementById('bulk-order-modal').remove()" class="absolute top-6 right-6 w-8 h-8 rounded-full bg-slate-50 hover:bg-slate-100 flex items-center justify-center transition-all text-slate-400 hover:text-bosch-blue">
            ...
            <form id="bulk-order-form" class="space-y-4">
                ...
                <button type="submit" class="w-full h-11 rounded-full bg-bosch-blue text-white font-black text-[10px] uppercase tracking-widest shadow-lg hover:bg-industrial-gray transition-colors">Generate Quotation Cart</button>
            </form>
        </div>
    `;
                ...
                <main class="flex-1 m-4 lg:m-6 p-6 lg:p-10 bg-white rounded-[2.5rem] shadow-sm border border-slate-200">
                    <div class="max-w-6xl mx-auto space-y-12 animate-fade-in">
                        <div class="flex justify-between items-end">
                            ...
                            <button onclick="app.renderCatalog(document.getElementById('view-container'))" class="px-8 py-3.5 bg-bosch-blue text-white font-black text-[10px] uppercase tracking-widest rounded-full hover:bg-industrial-gray transition-all shadow-lg shadow-blue-900/20">Add More Spares</button>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            <div class="card p-8 group border border-slate-200 rounded-3xl shadow-sm">
                                ...
                                <div class="flex gap-3">
                                    <button class="flex-1 h-12 rounded-full bg-bosch-blue text-white font-black text-[10px] uppercase tracking-widest hover:bg-industrial-gray transition-colors">Add to Cart</button>
