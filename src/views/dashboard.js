            ...
            <main class="flex-1 m-4 lg:m-6 p-6 lg:p-10 bg-white rounded-[2.5rem] shadow-sm border border-slate-200 space-y-12">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                    ${[
                        ...
                        <div class="bg-white rounded-none p-8 ring-1 ring-slate-900/5 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_40px_rgb(0,0,0,0.08)] group transition-all duration-500 hover:-translate-y-2 relative overflow-hidden">
                            <div class="absolute -right-4 -top-4 w-28 h-28 bg-${escapeHTML(s.c)}-50 rounded-none blur-2xl opacity-50 group-hover:scale-150 transition-transform duration-700"></div>
                            ...
                            <div class="flex items-end justify-between relative z-10">
                                ...
                                <span class="text-xs font-black text-${escapeHTML(s.c)}-700 bg-${escapeHTML(s.c)}-50/50 backdrop-blur-sm px-3 py-1.5 rounded-none border border-${escapeHTML(s.c)}-100/50 shadow-sm">${escapeHTML(s.s)}</span>
                            </div>
                        </div>
                    `).join('')}
                </div>
                
                <div class="bg-slate-50 rounded-[2.5rem] p-12 text-slate-900 relative overflow-hidden shadow-[0_20px_50px_rgb(0,0,0,0.15)] ring-1 ring-white/10">
                    ...
                    <div class="relative z-10 max-w-2xl">
                        ...
                        <div class="flex flex-wrap gap-4">
                            <button onclick="app.renderBulkOrderModal()" class="px-8 py-4 bg-white text-bosch-blue rounded-none font-black text-xs uppercase tracking-widest shadow-xl hover:scale-105 transition-transform flex items-center gap-2">
                            ...
                            <button onclick="app.renderCatalog(document.getElementById('view-container'))" class="px-8 py-4 bg-bosch-blue text-white rounded-none font-black text-xs uppercase tracking-widest shadow-xl shadow-slate-900/30 hover:bg-slate-700 transition-colors">Browse Catalog Manually</button>
                        </div>
                    </div>
                </div>
                </div>
...
export function renderBulkOrderModal(app) {
    ...
    modal.innerHTML = `
        <div class="bg-white rounded-none w-full max-w-xl p-8 space-y-6 shadow-2xl border border-slate-200 relative animate-in zoom-in-95 duration-300">
            <button onclick="document.getElementById('bulk-order-modal').remove()" class="absolute top-6 right-6 w-8 h-8 rounded-none bg-slate-50 hover:bg-slate-100 flex items-center justify-center transition-all text-slate-500 hover:text-bosch-blue">
            ...
            <form id="bulk-order-form" class="space-y-4">
                <div class="border-[2px] border-dashed border-slate-200 rounded-none p-8 text-center hover:border-bosch-blue hover:bg-bosch-blue/5 transition-all cursor-pointer relative group">
                    <input type="file" name="order_csv" accept=".csv" required class="font-sans absolute inset-0 opacity-0 cursor-pointer z-10" onchange="document.getElementById('csv-filename').textContent = this.files[0] ? this.files[0].name : 'Drop your CSV file here or browse'">
                ...
                <button type="submit" class="w-full h-11 rounded-none bg-bosch-blue text-white font-black text-xs uppercase tracking-widest shadow-lg hover:bg-industrial-gray transition-colors">Generate Quotation Cart</button>
            </form>
        </div>
    `;
...
export async function renderMyPartsList(container, app) {
    ...
    setHTML(container, `<div class="flex justify-center p-20"><div class="animate-spin w-10 h-10 border-4 border-emerald-600 border-t-transparent rounded-none"></div></div>`);
    ...
    try {
        ...
        const partsHtml = parts.length > 0 ? parts.map(p => `
            <div class="card p-8 group border border-slate-200 rounded-none shadow-sm bg-white relative">
                <div class="w-full h-48 bg-slate-50 rounded-none mb-6 overflow-hidden border border-slate-100 flex items-center justify-center relative">
                    ${p.stock_quantity <= 0 ? `<div class="absolute top-3 left-3 bg-rose-500 text-white text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-none shadow-sm">Out of Stock</div>` : ''}
                ...
                <div class="flex gap-3">
                    <button onclick='app.addToCart(${JSON.stringify({id: p.id, part_name: p.part_name, quantity: 1})})' class="flex-1 h-12 rounded-none bg-[#111111] hover:bg-[#ed1c24] text-white font-black text-[10px] uppercase tracking-widest transition-all duration-300 shadow-md hover:shadow-[#ed1c24]/20 transform hover:-translate-y-0.5 border border-transparent">Add to Cart</button>
                    <button onclick="window.removeFromPartsList(${p.id})" class="w-12 h-12 rounded-none border-2 border-slate-200 flex items-center justify-center text-rose-500 hover:bg-rose-50 hover:border-rose-200 transition-colors"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>
                </div>
            </div>
        `).join('') : `
            <div class="col-span-full py-8 text-center">
                <div class="w-24 h-24 bg-slate-50 rounded-none flex items-center justify-center mx-auto mb-6 border border-slate-100">
                ...
                <button onclick="app.renderCatalog(document.getElementById('view-container'))" class="px-8 py-3.5 bg-[#111111] hover:bg-[#ed1c24] text-white font-black text-[11px] uppercase tracking-widest rounded-none transition-all duration-300 shadow-xl shadow-[#111111]/10 hover:shadow-[#ed1c24]/20 transform hover:-translate-y-0.5">Browse Spares</button>
            </div>
        `;
                        ...
                        <div class="flex justify-between items-end border-b border-slate-100 pb-8">
                            <div>
                                <div class="flex items-center gap-3 mb-4">
                                    <div class="w-12 h-12 bg-[#ed1c24]/10 rounded-none flex items-center justify-center border border-[#ed1c24]/20">
                            ...
                            <button onclick="app.renderCatalog(document.getElementById('view-container'))" class="hidden sm:flex px-8 py-4 bg-[#111111] hover:bg-[#ed1c24] text-white font-black text-[11px] uppercase tracking-widest rounded-none transition-all duration-300 shadow-xl shadow-[#111111]/10 hover:shadow-[#ed1c24]/20 transform hover:-translate-y-0.5 items-center gap-2">
