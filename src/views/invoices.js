            ...
            <div class="flex flex-col lg:flex-row min-h-[calc(100vh-80px)] bg-slate-50">
                ...
                <main class="flex-1 m-4 lg:m-6 p-6 lg:p-10 bg-white rounded-[2.5rem] shadow-sm border border-slate-200">
                    <div class="max-w-6xl mx-auto space-y-12 animate-fade-in">
                        <div>
                            <div class="text-xs font-black uppercase tracking-[0.3em] text-bosch-blue">Financial Records</div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                ...
                                <div class="bg-white border-2 border-slate-100 rounded-none p-8 space-y-6 hover:border-bosch-blue transition-all duration-500 group">
                                    <div class="flex justify-between items-start">
                                        ...
                                        <div class="text-right space-y-2">
                                            ...
                                            <p class="text-xs font-black text-slate-500 uppercase tracking-widest">${new Date(inv.created_at).toLocaleDateString()}</p>
                                    ...
                                    <div class="space-y-4 pt-6 border-t border-slate-100">
                                        ${inv.tracking_number ? `
                                            <div class="p-4 bg-slate-50 rounded-none border border-slate-100">
                                                <p class="text-xs font-black text-slate-500 uppercase tracking-widest mb-1">Tracking Number</p>
                            ...
                            `).join('') : `
                                <div class="col-span-full bg-slate-50 border border-slate-100 rounded-none p-20 text-center text-slate-500 font-bold uppercase tracking-widest">No invoices generated yet.</div>
                            `}
                        </div>
                    </div>
                </main>
            ...
            <div id="invoice-printable-area" class="bg-white text-bosch-blue w-full max-w-4xl min-h-[11in] p-12 md:p-16 shadow-2xl relative animate-in slide-in-from-bottom-8 duration-500 rounded-none overflow-hidden mx-auto print:shadow-none print:p-0">
                ...
                <div class="relative z-10 flex flex-col md:flex-row justify-between items-start border-b-[3px] border-slate-900 pb-10 mt-6 print:mt-4 print:pb-8">
                    ...
                    <div class="text-right space-y-2 mt-8 md:mt-0">
                        ...
                        <div class="inline-block bg-slate-50 border border-slate-200 rounded-none p-4 mt-2 print:border-none print:bg-transparent print:p-0">
                            ...
                            <p class="text-xs font-bold text-slate-500 uppercase tracking-widest pt-1">Date: ${new Date(inv.created_at).toLocaleDateString()}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-10 py-8 print:py-8">
                    ...
                    <div class="md:text-right">
                        <p class="text-xs font-black text-slate-500 uppercase tracking-widest mb-3 border-b border-slate-100 pb-2">Payment Info:</p>
                        <div class="space-y-2">
                            <span class="inline-block px-4 py-1.5 bg-industrial-gray text-white rounded-none text-xs font-black uppercase tracking-widest border border-bosch-blue print:bg-transparent print:border-slate-300 print:text-slate-800">Payment on Delivery</span>
                            <p class="text-xs font-bold text-slate-500 uppercase tracking-widest pt-1">Status: <span class="${inv.status === 'delivered' ? 'text-bosch-blue' : 'text-amber-600'}">${escapeHTML(inv.status)}</span></p>
                        </div>
                    </div>
                </div>
                ...
                <div class="relative z-10 border-[1.5px] border-slate-900 rounded-none overflow-hidden mb-10 print:border-slate-300">
                    <table class="w-full text-left">
                        <thead class="bg-slate-900 text-white print:bg-slate-100 print:text-bosch-blue print:border-b-2 print:border-slate-900">
                            <tr>
                                <th class="p-4 md:p-5 text-xs font-black uppercase tracking-widest">Description</th>
                                <th class="p-4 md:p-5 text-xs font-black uppercase tracking-widest text-center">Qty</th>
                                <th class="p-4 md:p-5 text-xs font-black uppercase tracking-widest text-right">Unit Price</th>
                                <th class="p-4 md:p-5 text-xs font-black uppercase tracking-widest text-right bg-slate-800 print:bg-slate-200">Total</th>
                            </tr>
                        </thead>
                            ...
                            ${inv.items.map(item => `
                                <tr class="hover:bg-slate-50 print:hover:bg-transparent">
                                    <td class="p-4 md:p-6">
                                        ...
                                        <p class="text-xs text-slate-500 font-bold uppercase tracking-tight mt-1">${escapeHTML(item.brand)} • ${escapeHTML(item.machine_model)}</p>
                                    </td>
                ...
                <div class="relative z-10 flex flex-col md:flex-row justify-between items-start border-t-[3px] border-slate-900 pt-8 mt-auto print:border-slate-300">
                    <div class="mb-8 md:mb-0 max-w-xs space-y-2">
                        <p class="text-xs font-black text-slate-500 uppercase tracking-widest">Terms & Conditions:</p>
                        <p class="text-xs font-bold text-slate-500 leading-relaxed text-justify">All spare parts are subject to standard warranty conditions. Returns are accepted within 7 days of delivery if parts are unused and in original packaging. This is a computer generated invoice.</p>
                    </div>
                ...
                <div class="relative z-10 mt-16 pt-10 border-t border-slate-200 flex flex-col md:flex-row justify-between items-end print:mt-12">
                    <div class="space-y-4 mb-8 md:mb-0">
                        <h4 class="text-xs font-black text-bosch-blue uppercase tracking-widest">Authorised Signatory</h4>
