        ...
        <div class="flex flex-col lg:flex-row min-h-[calc(100vh-80px)] bg-[#0B1120]">
            <div class="hidden lg:block w-72 border-r border-white/5 p-8 space-y-8 animate-pulse bg-white/[0.02]">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 bg-white/10 rounded-none"></div>
                    <div class="space-y-2 flex-1"><div class="h-4 bg-white/10 rounded-none w-1/2"></div><div class="h-3 bg-white/10 rounded-none w-1/3"></div></div>
                </div>
                <div class="space-y-4 pt-4">
                    <div class="h-12 bg-white/5 rounded-none w-full"></div>
                    <div class="h-12 bg-white/5 rounded-none w-full"></div>
                    <div class="h-12 bg-white/5 rounded-none w-full"></div>
                </div>
            </div>
            <div class="flex-1 p-8 lg:p-12 space-y-10 animate-pulse">
                <div class="space-y-4">
                    <div class="h-10 bg-white/10 rounded-none w-1/4"></div>
                    <div class="h-6 bg-white/5 rounded-none w-1/3"></div>
                </div>
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-6">
                    <div class="h-36 bg-white/5 rounded-none w-full"></div>
                    <div class="h-36 bg-white/5 rounded-none w-full"></div>
                    <div class="h-36 bg-white/5 rounded-none w-full"></div>
                    <div class="h-36 bg-white/5 rounded-none w-full"></div>
                </div>
                <div class="h-64 bg-white/5 rounded-none w-full"></div>
            </div>
        </div>
    `);
    ...
    try {
        ...
        setHTML(container, `
            <div class="flex flex-col lg:flex-row min-h-[calc(100vh-80px)] bg-[#0B1120] text-slate-200 selection:bg-blue-500/30 font-sans relative overflow-hidden">
                ...
                <div class="absolute top-[-20%] left-[-10%] w-[50%] h-[50%] rounded-none bg-blue-600/10 blur-[120px] pointer-events-none"></div>
                <div class="absolute bottom-[-10%] right-[-5%] w-[40%] h-[40%] rounded-none bg-emerald-600/10 blur-[120px] pointer-events-none"></div>
                ...
                <main class="flex-1 p-6 lg:p-12 space-y-10 relative z-10 overflow-y-auto h-[calc(100vh-80px)] no-scrollbar">
                    ...
                    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 group">
                        <div>
                            <div class="inline-flex items-center gap-2 px-3 py-1.5 rounded-none bg-blue-500/10 border border-blue-500/20 text-blue-400 text-xs font-black uppercase tracking-[0.2em] mb-4">
                                <span class="w-2 h-2 rounded-none bg-blue-500 animate-pulse shadow-[0_0_8px_rgba(59,130,246,0.8)]"></span>
                                Live Workstation
                            </div>
                    ...
                    <div class="grid grid-cols-2 lg:grid-cols-4 gap-6">
                        ${[
                            ...
                            '<div class="relative group bg-white/5 backdrop-blur-xl border border-white/10 rounded-none p-6 hover:-translate-y-1.5 transition-all duration-300 hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.5)] overflow-hidden cursor-default">' +
                                ...
                                '<div class="relative z-10 flex flex-col h-full justify-between gap-4">' +
                                    '<div class="w-12 h-12 rounded-none bg-white/5 border border-white/10 flex items-center justify-center group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300 shadow-inner">' +
                    ...
                    ${low_stock && low_stock.length > 0 ? `
                        <div class="relative overflow-hidden bg-rose-500/10 border border-rose-500/20 backdrop-blur-md rounded-none p-8">
                            <div class="absolute top-0 right-0 w-64 h-64 bg-rose-500/20 blur-[80px] rounded-none pointer-events-none"></div>
                            <div class="relative z-10">
                                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                                    <div class="flex items-center gap-4">
                                        <div class="w-14 h-14 rounded-none bg-rose-500/20 border border-rose-500/30 flex items-center justify-center animate-pulse shadow-[0_0_15px_rgba(244,63,94,0.3)]">
                                    ...
                                    <button onclick="app.renderStockAdjustModal()" class="px-6 py-3.5 bg-rose-500 hover:bg-rose-400 text-white rounded-none font-black text-xs uppercase tracking-[0.2em] transition-all shadow-lg shadow-rose-500/20 hover:shadow-rose-500/40 hover:-translate-y-0.5 border border-rose-400/50">
                                        Mass Adjust
                                    </button>
                                </div>
                                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                    ${low_stock.map(item => 
                                        '<div class="bg-[#0B1120]/50 border border-rose-500/10 rounded-none p-5 hover:bg-rose-500/10 transition-colors group">' +
                                            ...
                                            '<div class="mt-4 flex items-center justify-between">' +
                                                '<div class="px-3 py-1 rounded-none ' + (item.stock_quantity <= 0 ? 'bg-rose-500/20 text-rose-400 border border-rose-500/30' : 'bg-amber-500/20 text-amber-400 border border-amber-500/30') + ' text-xs font-black">' + escapeHTML(String(item.stock_quantity)) + ' left</div>' +
                                                '<button onclick="app.renderStockAdjustModal(' + item.id + ')" class="w-8 h-8 rounded-none bg-white/5 hover:bg-rose-500 text-slate-400 hover:text-white flex items-center justify-center transition-all group/btn"><svg class="w-4 h-4 group-hover/btn:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 4v16m8-8H4"/></svg></button>' +
                                            '</div>' +
                                        '</div>'
                                    ).join('')}
                                </div>
                    ...
                    ` : `
                        <div class="bg-emerald-500/10 border border-emerald-500/20 backdrop-blur-md rounded-none p-6 flex items-center gap-5">
                            <div class="w-14 h-14 rounded-none bg-emerald-500/20 border border-emerald-500/30 flex items-center justify-center flex-shrink-0 shadow-[0_0_15px_rgba(16,185,129,0.3)]">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div class="space-y-4">
                            ...
                            <button onclick="app.renderStockAdjustModal()"
                                class="w-full relative overflow-hidden bg-white/[0.03] border border-white/10 rounded-none p-6 text-left hover:bg-blue-500/10 hover:border-blue-500/30 transition-all duration-300 group">
                                <div class="flex items-center gap-5 relative z-10">
                                    <div class="w-12 h-12 rounded-none bg-blue-500/10 border border-blue-500/20 text-blue-400 flex items-center justify-center group-hover:scale-110 group-hover:bg-blue-500 group-hover:text-white transition-all shadow-[0_0_15px_rgba(59,130,246,0.0)] group-hover:shadow-[0_0_20px_rgba(59,130,246,0.4)]">
                            ...
                            <a href="/admin/stock-logs" data-link
                                class="w-full relative overflow-hidden bg-white/[0.03] border border-white/10 rounded-none p-6 text-left hover:bg-emerald-500/10 hover:border-emerald-500/30 transition-all duration-300 group block">
                                <div class="flex items-center gap-5 relative z-10">
                                    <div class="w-12 h-12 rounded-none bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 flex items-center justify-center group-hover:scale-110 group-hover:bg-emerald-500 group-hover:text-white transition-all shadow-[0_0_15px_rgba(16,185,129,0.0)] group-hover:shadow-[0_0_20px_rgba(16,185,129,0.4)]">
                            ...
                            <a href="/admin/reports" data-link
                                class="w-full relative overflow-hidden bg-white/[0.03] border border-white/10 rounded-none p-6 text-left hover:bg-amber-500/10 hover:border-amber-500/30 transition-all duration-300 group block">
                                <div class="flex items-center gap-5 relative z-10">
                                    <div class="w-12 h-12 rounded-none bg-amber-500/10 border border-amber-500/20 text-amber-400 flex items-center justify-center group-hover:scale-110 group-hover:bg-amber-500 group-hover:text-white transition-all shadow-[0_0_15px_rgba(245,158,11,0.0)] group-hover:shadow-[0_0_20px_rgba(245,158,11,0.4)]">
                        ...
                        <!-- Activity -->
                        <div class="lg:col-span-2 bg-white/[0.02] border border-white/5 rounded-none overflow-hidden backdrop-blur-xl flex flex-col">
                            <div class="p-7 border-b border-white/5 flex items-center justify-between">
                                <div class="flex items-center gap-3">
                                    <div class="w-2 h-6 bg-blue-500 rounded-none shadow-[0_0_10px_rgba(59,130,246,0.6)]"></div>
                                    <h3 class="text-lg font-black text-white tracking-tight">Recent Activity</h3>
                                </div>
                                <a href="/admin/stock-logs" data-link class="px-4 py-2 rounded-none bg-white/5 hover:bg-white/10 text-xs font-bold text-slate-300 hover:text-white uppercase tracking-wider transition-all">View All</a>
                            </div>
                            <div class="flex-1 overflow-y-auto no-scrollbar p-3">
                                ${recentLogs.length > 0 ? `
                                    <div class="space-y-1">
                                        ${recentLogs.map(log => `
                                            <div class="p-4 rounded-none flex items-center justify-between hover:bg-white/5 transition-colors group">
                                                <div class="flex items-center gap-4">
                                                    <div class="w-10 h-10 rounded-none flex items-center justify-center border ${log.type === 'in' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-rose-500/10 border-rose-500/20 text-rose-400'}">
                                                ...
                                                <div class="text-right">
                                                    <span class="inline-flex items-center justify-center px-3 py-1 rounded-none text-sm font-black border ${log.type === 'in' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-rose-500/10 border-rose-500/20 text-rose-400'}">
        ...
        setHTML(container, `
            <div class="flex flex-col items-center justify-center min-h-[calc(100vh-80px)] bg-[#0B1120] p-10 text-center relative overflow-hidden">
                ...
                <button onclick="window.location.reload()" class="mt-8 px-8 py-4 bg-white/5 hover:bg-white/10 text-white rounded-none font-black text-xs uppercase tracking-[0.2em] transition-all border border-white/10 hover:border-white/20 relative z-10">
                    Reboot Terminal
                </button>
            </div>
        `);
...
function getStaffSidebar(app) {
    ...
    const link = (href, label, icon, active) => 
        '<a href="' + escapeHTML(href) + '" data-link class="relative flex items-center gap-4 px-5 py-4 rounded-none transition-all duration-300 font-bold text-xs uppercase tracking-[0.15em] group overflow-hidden ' + 
        (active ? 'text-white' : 'text-slate-400 hover:text-white') + '">' +
            (active ? '<div class="absolute inset-0 bg-blue-500/20 border border-blue-500/30 rounded-none shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)]"></div>' : '<div class="absolute inset-0 bg-white/5 opacity-0 group-hover:opacity-100 transition-opacity rounded-none"></div>') +
            ...
            (active ? '<div class="absolute right-3 w-1.5 h-1.5 rounded-none bg-blue-400 shadow-[0_0_10px_rgba(96,165,250,1)]"></div>' : '') +
        '</a>';
    ...
    return `
        <aside class="hidden lg:flex w-72 bg-[#0B1120]/80 backdrop-blur-3xl border-r border-white/5 flex-col sticky top-20 h-[calc(100vh-80px)] z-20">
            <div class="p-8 border-b border-white/5 bg-white/[0.01]">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 rounded-none bg-gradient-to-br from-blue-500 to-blue-700 text-white flex items-center justify-center font-black text-xl shadow-[0_0_20px_rgba(37,99,235,0.4)] border border-blue-400/30 relative">
                        ${userName.charAt(0)}
                        <div class="absolute -bottom-1 -right-1 w-4 h-4 rounded-none bg-emerald-500 border-2 border-[#0B1120]"></div>
                    </div>
                    <div>
            ...
            <div class="p-6 border-t border-white/5 bg-white/[0.01]">
                <a href="/logout" data-link class="flex items-center justify-center gap-3 w-full px-5 py-4 rounded-none text-rose-400 hover:text-white hover:bg-rose-500/20 transition-all font-black text-[11px] uppercase tracking-[0.2em] group border border-transparent hover:border-rose-500/30 relative overflow-hidden">
