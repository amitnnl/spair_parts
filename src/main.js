...
const app = {
    ...
    applySettings() {
        ...
        if (footerSiteName) {
            const rawName = s.site_name || 'TORVO TOOLS PRIVATE LIMITED';
            const match = rawName.match(/(.*)\b(private limited|pvt\.? ltd\.?)\b/i);
            if (match) {
                const mainPart = match[1].trim();
                const redPart = match[2].trim();
                setHTML(footerSiteName, escapeHTML(mainPart) + ' <span class="text-[#ed1c24]">' + escapeHTML(redPart) + '</span>');
            } else {
                ...
                if (words.length > 1) {
                    ...
                    setHTML(footerSiteName, '<span class="text-[#ed1c24]">' + escapeHTML(rawName) + '</span>');
                }
            }
        }
        ...
        if (this.state.user) {
            ...
            setHTML(authContainer, `
                <div class="relative group/user z-50">
                    ...
                    <div class="absolute right-0 top-full mt-2 w-48 bg-white border border-zinc-200 shadow-xl opacity-0 invisible group-hover/user:opacity-100 group-hover/user:visible transition-all duration-200 z-50 py-2 rounded-none">
                        ...
                        ` + (isAdmin ? `
                            <a href="/admin" data-link class="block px-4 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-50 hover:text-[#ed1c24] transition-colors uppercase tracking-wider flex items-center justify-end gap-1.5">
                                Admin Panel
                        ...
                        ` : this.state.user?.role?.toLowerCase() === 'staff' ? `
                            <a href="/staff" data-link class="block px-4 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-50 hover:text-[#ed1c24] transition-colors uppercase tracking-wider flex items-center justify-end gap-1.5">
                                Staff Panel
                        ...
                        ` : `
                            <a href="/dashboard" data-link class="block px-4 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-50 hover:text-[#ed1c24] transition-colors uppercase tracking-wider flex items-center justify-end gap-1.5">
                                Dashboard
                        ...
                        `) + (!isAdmin ? `
                        <a href="/profile" data-link class="block px-4 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-50 hover:text-[#ed1c24] transition-colors uppercase tracking-wider text-right">My Profile</a>
                        ` : '') + `
                        <a href="/logout" data-link class="block px-4 py-2.5 text-xs font-bold text-rose-600 hover:bg-rose-50 transition-colors uppercase tracking-wider border-t border-zinc-100 mt-1 flex items-center justify-end gap-1.5">
                            Log Out
            ...
            setHTML(authContainer, `
                <div class="relative group/user z-50">
                    ...
                    <div class="absolute right-0 top-full mt-2 w-48 bg-white border border-zinc-200 shadow-xl opacity-0 invisible group-hover/user:opacity-100 group-hover/user:visible transition-all duration-200 z-50 py-2 rounded-none">
                        <a href="/login" data-link class="block px-4 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-50 hover:text-[#ed1c24] transition-colors uppercase tracking-wider text-right">Log In</a>
                        <a href="/register" data-link class="block px-4 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-50 hover:text-[#ed1c24] transition-colors uppercase tracking-wider text-right">Sign Up</a>
                    </div>
                </div>
            `);
        }
